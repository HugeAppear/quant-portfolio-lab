#!/usr/bin/env python3
"""Run a v0.1 backtest and emit the result page artifacts.

Loads price + fundamental data from DuckDB when available, otherwise falls back
to synthetic data (or pass ``--synthetic`` to force it). Prints the headline
metrics, writes the three result charts to HTML, and persists the run to the
``backtest_runs`` / ``backtest_positions`` / ``backtest_trades`` tables.

Examples
--------
    python scripts/run_backtest.py --synthetic --strategy low_pbr --rebalance 1Y --top-n 20
    python scripts/run_backtest.py --strategy low_per --rebalance 6M --benchmark KOSPI
"""
...
STRATEGY_FACTOR = {"low_pbr": "pbr", "low_per": "per"}
...
def _load_inputs(args):
    ...
    if not args.synthetic and Path(args.db).exists():
        ...
        if not panel.empty and not fundamentals.empty:
            ...
            print(f"[run_backtest] price panel: {panel.shape[1]} assets x {panel.shape[0]} trading days")
    ...
    print(f"[run_backtest] synthetic price panel: {panel.shape[1]} assets x {panel.shape[0]} trading days")
...
def _persist_run(db, result, args, run_id):
    ...
    con.execute(
        ...
        [
            run_id, args.strategy, "kr_common_equity", args.rebalance, args.benchmark,
            args.fee, f"sell_tax={args.tax}", f"flat={args.slippage}",
            "100pct_loss", datetime.now(UTC).replace(tzinfo=None),
        ],
    )
    if not result.positions.empty:
        pos = result.positions.copy()
        pos.insert(0, "run_id", run_id)
        con.register("_pos", pos[["run_id", "date", "asset_id", "weight",
    ...
    if not result.trades.empty:
        ...
        con.register("_trd", trd[["run_id", "date", "asset_id", "side", "quantity",
                                  "execution_price", "gross_notional", "commission",
                                  "transaction_tax", "slippage_cost"]])
...
def main() -> int:
    ...
    parser.add_argument("--strategy", choices=list(STRATEGY_FACTOR), default="low_pbr")
    ...
    panel, fundamentals, bench = _load_inputs(args)

    engine = BacktestEngine(
        panel,
        CostModel(fee_rate=args.fee, tax_rate=args.tax, slippage_rate=args.slippage),
        fundamentals=fundamentals,
        benchmark=bench,
        config=BacktestConfig(
            initial_capital=args.capital, rebalance_mode=args.rebalance,
            top_n=args.top_n, factor=STRATEGY_FACTOR[args.strategy],
